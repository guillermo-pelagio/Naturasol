namespace CapaEntidades
{
    public class OrdenCompra
    {
        public string U_LiberacionOC
        {
            get; set;
        }

        public string DocEntry
        {
            get; set;
        }

        public string DocNum
        {
            get; set;
        }

        public string Sociedad
        {
            get; set;
        }

        public string CardCode
        {
            get; set;
        }

        public string CardName
        {
            get; set;
        }

        public string DocDate
        {
            get; set;
        }

        public string DocType
        {
            get; set;
        }

        public string UserSign
        {
            get; set;
        }

        public string SlpName
        {
            get; set;
        }
        public string TaxDate
        {
            get; set;
        }
        public string DocTotal
        {
            get; set;
        }
        public string DocCur
        {
            get; set;
        }
        public string Pagado
        {
            get; set;
        }
        public string Pendiente
        {
            get; set;
        }
        public string Anticipo
        {
            get; set;
        }
        public string Email
        {
            get; set;
        }
        public string PendienteAnticipo
        {
            get; set;
        }
        public string DocRate
        {
            get; set;
        }
        public string VatSum
        {
            get; set;
        }
        public string VatSumFC
        {
            get; set;
        }
        public string DocTotalFC
        {
            get; set;
        }
        public string ItemCode
        {
            get; set;
        }
        public string Dscription
        {
            get; set;
        }
        public string AcctCode
        {
            get; set;
        }
        public string AcctName
        {
            get; set;
        }
        public string U_CantidadServicio
        {
            get; set;
        }
        public string Quantity
        {
            get; set;
        }
        public string LineNum
        {
            get; set;
        }
        public string WhsCode
        {
            get; set;
        }
        public string Currency
        {
            get; set;
        }
        public string Price
        {
            get; set;
        }
        public string LineTotal
        {
            get; set;
        }
        public string TotalFrgn
        {
            get; set;
        }
        public string TaxCode
        {
            get; set;
        }

        public string trgtPath
        {
            get; set;
        }

        public string WddCode
        {
            get; set;
        }

        public string WtmCode
        {
            get; set;
        }

        public string OcrCode
        {
            get; set;
        }

        public string OcrCode3
        {
            get; set;
        }

        public string NumAtCard
        {
            get; set;
        }

        public string OnHand
        {
            get; set;
        }

        public string Consumo
        {
            get; set;
        }

        public string Inventario1
        {
            get; set;
        }

        public string Inventario2
        {
            get; set;
        }

        public string ShipDate
        {
            get; set;
        }        
    }
}
