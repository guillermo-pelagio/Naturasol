...
namespace ServicioCostosProduccion
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                bool bandera = true;
                if (DateTime.Now.Minute % 5 == 0 && bandera == true && DateTime.Now.Second > 15 && DateTime.Now.Second < 30)
                {
                    //for (int i = 0; i < basesDatos.Length; i++)
                    {
                        List<OrdenFabricacion> listaOFAbiertas = new List<OrdenFabricacion>();
                        listaOFAbiertas = ordenFabricacionBLL.obtenerLiberacionesCosto();

                        if (listaOFAbiertas.Count > 0)
                        {
                            for (int j = 0; j < listaOFAbiertas.Count; j++)
                            {
                                DIAPIBLL.conectarDIAPI("TSSL_" + listaOFAbiertas[j].Sociedad);
                                string body = "";
