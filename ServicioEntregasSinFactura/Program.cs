        ...
        static void Main(string[] args)
        {
            ...
            if (listaEntregasPendientes.Count > 0)
            {
                for (int m = 0; m < listaEntregasPendientes.Count; m++)
                {
                    ...
                    body = body.Replace("valorsociedad", listaEntregasPendientes[m].Sociedad);
                    body = body.Replace("valorfolio", listaEntregasPendientes[m].DocNum);
                    body = body.Replace("valorcliente", listaEntregasPendientes[m].CardCode + " - " + listaEntregasPendientes[m].CardName);
                    body = body.Replace("valorreferencia", listaEntregasPendientes[m].NumAtCard);
                    body = body.Replace("valormonto", string.Format("{0:C}", Convert.ToDecimal(listaEntregasPendientes[m].DocTotal)));
                    body = body.Replace("valorfecha", Convert.ToDateTime(listaEntregasPendientes[m].DocDate).ToShortDateString());
                }
            }
            ...
            if (enviaCorreo)
            {
                ...
                correoBLL.enviarCorreo("Entrega sin Factura +24 hrs", body, "rodrigo.lozano@naturasol.com.mx, alma.padilla@naturasol.com.mx", "andres.sanchez@naturasol.com.mx, angel.santoyo@naturasol.com.mx, jenifer.castillo@naturasol.com.mx, noe.orozco@naturasol.com.mx, jose.chacon@naturasol.com.mx");
            }
        }
    }
}
