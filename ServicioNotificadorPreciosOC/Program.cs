...
namespace ServicioNotificadorPreciosOC
{
    class Program
    {
        public static void Main(string[] args)
        {
            ListaPreciosBLL listaPreciosBLL = new ListaPreciosBLL();
            ...
            {
                List<ListaPrecios> listaDocumentosCompra = new List<ListaPrecios>();
                listaDocumentosCompra = listaPreciosBLL.obtenerDocumentosCompra(basesDatos[i]);

                if (listaDocumentosCompra.Count > 0)
                {
                    DIAPIBLL.conectarDIAPI(basesDatos[i]);
                    for (int j = 0; j < listaDocumentosCompra.Count; j++)
                    {
                        string diferencias = "";
                        string errorCodigo = "";
                        if ((listaDocumentosCompra[j].PrecioLE == null) || (listaDocumentosCompra[j].PrecioLE == ""))
                        {
                            //CORREO Y ACTUALIZAMOS A OK
                            diferencias = diferencias + "\nCODIGO: " + listaDocumentosCompra[j].ItemCode + " (" + listaDocumentosCompra[j].Dscription + ") UM: " + listaDocumentosCompra[j].unitMsr + " PRECIO O.C: " + listaDocumentosCompra[j].Price + " SIN LISTA DE PRECIOS ESPECIAL.";
                            listaPreciosBLL.actualizarDocumentosCompra(listaDocumentosCompra[j].DocEntry, listaDocumentosCompra[j].VisOrder, listaDocumentosCompra[j].ItemCode, 0);
                        }
                        else
                        {
                            if (listaDocumentosCompra[j].unitMsr.Equals(listaDocumentosCompra[j].UomName))
                            {
                                if (Convert.ToDouble(listaDocumentosCompra[j].Price) == Convert.ToDouble(listaDocumentosCompra[j].PrecioLE))
                                {
                                    //ACTUALIZAMOS A OK
                                    listaPreciosBLL.actualizarDocumentosCompra(listaDocumentosCompra[j].DocEntry, listaDocumentosCompra[j].VisOrder, listaDocumentosCompra[j].ItemCode, 1);
                                }
                                else
                                {
                                    //CORREO Y ACTUALIZAMOS A OK
                                    diferencias = diferencias + "\nCODIGO: " + listaDocumentosCompra[j].ItemCode + " (" + listaDocumentosCompra[j].Dscription + ") UM: " + listaDocumentosCompra[j].unitMsr + " PRECIO O.C: " + listaDocumentosCompra[j].Price + " PRECIO LISTA: " + listaDocumentosCompra[j].PrecioLE;
                                    listaPreciosBLL.actualizarDocumentosCompra(listaDocumentosCompra[j].DocEntry, listaDocumentosCompra[j].VisOrder, listaDocumentosCompra[j].ItemCode, 0);
                                }
                            }
                            else
                            {
                                if (Convert.ToDouble(listaDocumentosCompra[j].PrecioUOM) == Convert.ToDouble(listaDocumentosCompra[j].Price))
                                {
                                    //ACTUALIZAMOS A OK
                                    listaPreciosBLL.actualizarDocumentosCompra(listaDocumentosCompra[j].DocEntry, listaDocumentosCompra[j].VisOrder, listaDocumentosCompra[j].ItemCode, 1);
                                }
                                ...
                                {
                                    //CORREO Y ACTUALIZAMOS A OK
                                    diferencias = diferencias + "\nCODIGO: " + listaDocumentosCompra[j].ItemCode + " (" + listaDocumentosCompra[j].Dscription + ") UM: " + listaDocumentosCompra[j].unitMsr + " PRECIO O.C: " + listaDocumentosCompra[j].Price + " PRECIO LISTA: " + listaDocumentosCompra[j].PrecioUOM;
                                    listaPreciosBLL.actualizarDocumentosCompra(listaDocumentosCompra[j].DocEntry, listaDocumentosCompra[j].VisOrder, listaDocumentosCompra[j].ItemCode, 0);
                                }
                            }
                        }
                        ...
                        {
                            diferencias = "\n" + diferencias;
                            errorCodigo = "La O.C " + listaDocumentosCompra[j].DocNum + " (" + listaDocumentosCompra[j].CardName + ") tiene los siguientes códigos con precios unitarios diferentes al de la lista de precios: \n";
                            //correoBLL.enviarCorreo("O.C. Con precios diferentes", leerPlantilla(errorCodigo, diferencias), "jenifer.castillo@naturasol.com.mx", null);
                            correoBLL.enviarCorreo("O.C. Con precios diferentes", leerPlantilla(errorCodigo, diferencias), "guillermo.pelagio@naturasol.com.mx", null);
                        }
                    }
