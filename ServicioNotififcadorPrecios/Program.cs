...
namespace ServicioNotificadorPrecios
{
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            {
                List<ListaPrecios> listaDocumentosVenta = new List<ListaPrecios>();
                listaDocumentosVenta = listaPreciosBLL.obtenerDocumentosVenta(basesDatos[i]);

                if (listaDocumentosVenta.Count > 0)
                {
                    DIAPIBLL.conectarDIAPI(basesDatos[i]);
                    for (int j = 0; j < listaDocumentosVenta.Count; j++)
                    {
                        string diferencias = "";
                        string errorCodigo = "";
                        if ((listaDocumentosVenta[j].PrecioLE == null) || (listaDocumentosVenta[j].PrecioLE == ""))
                        {
                            //CORREO Y ACTUALIZAMOS A OK
                            diferencias = diferencias + "\nCODIGO: " + listaDocumentosVenta[j].ItemCode + " (" + listaDocumentosVenta[j].Dscription + ") UM: " + listaDocumentosVenta[j].unitMsr + " PRECIO O.V: " + listaDocumentosVenta[j].Price + " SIN LISTA DE PRECIOS ESPECIAL.";
                            listaPreciosBLL.actualizarDocumentosVenta(listaDocumentosVenta[j].DocEntry, listaDocumentosVenta[j].VisOrder, listaDocumentosVenta[j].ItemCode);
                        }
                        else
                        {
                            if (listaDocumentosVenta[j].unitMsr.Equals(listaDocumentosVenta[j].UomName))
                            {
                                if (Convert.ToDouble(listaDocumentosVenta[j].Price) == Convert.ToDouble(listaDocumentosVenta[j].PrecioLE))
                                {
                                    //ACTUALIZAMOS A OK
                                    listaPreciosBLL.actualizarDocumentosVenta(listaDocumentosVenta[j].DocEntry, listaDocumentosVenta[j].VisOrder, listaDocumentosVenta[j].ItemCode);
                                }
                                ...
                                {
                                    //CORREO Y ACTUALIZAMOS A OK
                                    diferencias = diferencias + "\nCODIGO: " + listaDocumentosVenta[j].ItemCode + " (" + listaDocumentosVenta[j].Dscription + ") UM: " + listaDocumentosVenta[j].unitMsr + " PRECIO O.V: " + listaDocumentosVenta[j].Price + " PRECIO LISTA: " + listaDocumentosVenta[j].PrecioLE;
                                    listaPreciosBLL.actualizarDocumentosVenta(listaDocumentosVenta[j].DocEntry, listaDocumentosVenta[j].VisOrder, listaDocumentosVenta[j].ItemCode);
                                }
                            }
                            else
                            {
                                if (Convert.ToDouble(listaDocumentosVenta[j].PrecioUOM) == Convert.ToDouble(listaDocumentosVenta[j].Price))
                                {
                                    //ACTUALIZAMOS A OK
                                    listaPreciosBLL.actualizarDocumentosVenta(listaDocumentosVenta[j].DocEntry, listaDocumentosVenta[j].VisOrder, listaDocumentosVenta[j].ItemCode);
                                }
                                ...
                                {
                                    //CORREO Y ACTUALIZAMOS A OK
                                    diferencias = diferencias + "\nCODIGO: " + listaDocumentosVenta[j].ItemCode + " (" + listaDocumentosVenta[j].Dscription + ") UM: " + listaDocumentosVenta[j].unitMsr + " PRECIO O.V: " + listaDocumentosVenta[j].Price + " PRECIO LISTA: " + listaDocumentosVenta[j].PrecioUOM;
                                    listaPreciosBLL.actualizarDocumentosVenta(listaDocumentosVenta[j].DocEntry, listaDocumentosVenta[j].VisOrder, listaDocumentosVenta[j].ItemCode);
                                }
                            }
                        }
                        ...
                        {
                            diferencias = "\n" + diferencias;
                            errorCodigo = "La O.V " + listaDocumentosVenta[j].DocNum + " (" + listaDocumentosVenta[j].CardName + ") tiene los siguientes códigos con precios unitarios diferentes al de la lista de precios: \n";
                            //correoBLL.enviarCorreo("O.V. Con precios diferentes", leerPlantilla(errorCodigo, diferencias), "jenifer.castillo@naturasol.com.mx", null);
                            correoBLL.enviarCorreo("O.V. Con precios diferentes", leerPlantilla(errorCodigo, diferencias), "guillermo.pelagio@naturasol.com.mx", null);
                        }
                    }
